...
async function execute(config) {
  return new Promise((resolve, reject) => {
    try {
      ...
      app.get('/api/facility-helpdesk/health', (req, res) => {
        const healthInfo = {
          status: 'ok',
          timestamp: new Date().toISOString().replace('T', ' ').replace(/\.\d{3}Z$/, ''),
          service: 'facility-helpdesk',
      ...
      app.get('/api/facility-helpdesk/info', (req, res) => {
        res.json({
      ...
      const modulesDir = isBunCompiled
        ? path.join(workingDir, 'src', 'modules', 'facility-helpdesk')
        : path.join(workingDir, 'facility-helpdesk');
      ...
      try {
        ...
        for (const file of endpointFiles) {
          try {
            ...
            const endpointPrefix = `/api/facility-helpdesk/${endpointName}`;
            ...
            try {
              ...
              if (exportConfig) {
                ExportHandler.registerRoutes(app, 'facility-helpdesk', endpointName, exportConfig);
                logger.info({
            ...
            try {
              ...
              if (importConfig) {
                ImportHandler.registerRoutes(app, 'facility-helpdesk', endpointName, importConfig);
                logger.info({
      ...
      const server = app.listen(port, serverAddress, (err) => {
        ...
        logServerReady({
          ...
          healthCheck: `http://${displayHost}:${port}/api/facility-helpdesk/health`,
          serviceInfo: `http://${displayHost}:${port}/api/facility-helpdesk/info`,
