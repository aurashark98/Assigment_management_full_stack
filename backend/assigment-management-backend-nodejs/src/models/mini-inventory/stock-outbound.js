...
/**
 * StockOutbound Model - Auto-generated on 2026-04-15 00:37:08
 *
 * Model untuk stock-outbound yang mewarisi fungsi-fungsi dari BaseModel
 * Table: stock_outbound
 * Fields: 11 fields
 * Database: PostgreSQL
 */
class StockOutboundModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel stock-outbound
    const validFields = [
      'stock_outbound_id',
      'outbound_number',
      'outbound_date',
    ...
    const datatablesWhere = ["outbound_number","warehouse_code","warehouse_name","customer_code","customer_name","reference_number","status","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('stock_outbound', validFields, datatablesWhere);

    // Setup primary key dari payload atau fallback ke fieldName pertama
    this.primaryKey = 'stock_outbound_id';

    // Setup viewName untuk operasi read jika berbeda dari tableName
    this.viewName = 'stock_outbound';
    this.readSource = 'stock_outbound'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'stock_outbound'; // Source untuk operasi write (add, update, delete)
    ...
    // Model metadata
    this.modelMetadata = {
      endpointName: 'stock-outbound',
      moduleName: 'mini-inventory',
      tableName: 'stock_outbound',
      viewName: 'stock_outbound',
      fieldCount: 11,
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan stock-outbound
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk stock-outbound
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'stock-outbound-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'stock_outbound_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT stock_outbound_id, stock_outbound_id FROM ' + this.getLookupSource() + ' WHERE upper(a.stock_outbound_id) LIKE upper($1) ORDER BY a.stock_outbound_id';
      ...
      const result = data.map(item => ({
        id: item.stock_outbound_id,
        text: item.stock_outbound_id
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (StockOutboundModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["stock_outbound_id","outbound_number","outbound_date","warehouse_id","customer_id","reference_number","notes","total_items","total_qty","total_amount","status"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'stock_outbound_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.stock_outbound_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (StockOutboundModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'stock_outbound_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT stock_outbound_id, stock_outbound_id FROM ' + this.getLookupSource() + ' ORDER BY a.stock_outbound_id';
      ...
      return data.map(item => {
        ...
        if (item.stock_outbound_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (StockOutboundModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['stock_outbound_id', 'stock_outbound_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = [];
      let selectClause = 'stock_outbound_id';
      let textField = 'stock_outbound_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'stock_outbound_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'stock_outbound_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'stock_outbound_id') return null;
      ...
      console.log('=== DEBUG STOCK-OUTBOUND LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.stock_outbound_id,
        text: item[textFieldName] || item.stock_outbound_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (StockOutboundModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: [],
      dateFields: ["outbound_date"],
      requiredFields: [],
      primaryTextField: 'stock_outbound_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk stock-outbound
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response stock-outbound
   */
  ...
  async createComposite(data, eventContext = null) {
    ...
    try {
      ...
      const detailKey = 'stock_outbound_item';
      ...
      // Insert detail items
      const insertedItems = [];
      const detailTableName = 'stock_outbound_item';
      const foreignKey = 'stock_outbound_id';
      ...
      for (const item of data[detailKey] || []) {
        ...
        // Auto-calculate total_amount = qty_shipped * unit_price
        item.total_amount = (Number(item.qty_shipped) || 0) * (Number(item.unit_price) || 0);
  ...
  async updateComposite(data, eventContext = null) {
    ...
    try {
      ...
      const detailTableName = 'stock_outbound_item';
      const foreignKey = 'stock_outbound_id';
      const detailPrimaryKey = 'stock_outbound_item_id';
      ...
      const calculations = {"total_items":{"type":"count","source":"items.length","description":"Total number of distinct items"},"total_qty":{"type":"sum","source":"items.qty_shipped","description":"Sum of all qty_shipped from items"},"total_amount":{"type":"sum","source":"items.total_amount","description":"Sum of all total_amount from items"}};
  ...
  async readComposite(options) {
    try {
      ...
      let detailQuery;
      try {
        const detailQueryFilePath = path.join(__dirname, 'query', 'stock-outbound-detailquery.sql');
  ...
  async deleteData(options, eventContext = null) {
    ...
    try {
      ...
      console.log('Deleting child records from stock_outbound_item...');

      // Get primary key value untuk delete child records
      const headerQuery = `SELECT stock_outbound_id FROM ${this.writeSource} WHERE ${whereClause}`;
      ...
      const headerPrimaryKeyValue = headerResult.rows[0]['stock_outbound_id'];

      // DELETE child records menggunakan foreignKey
      const deleteChildQuery = `DELETE FROM stock_outbound_item WHERE stock_outbound_id = $1`;
      const deleteChildResult = await client.query(deleteChildQuery, [headerPrimaryKeyValue]);

      console.log(`Deleted ${deleteChildResult.rowCount} child record(s) from stock_outbound_item`);
...
module.exports = new StockOutboundModel();