...
/**
 * Supplier Model - Auto-generated on 2026-04-15 00:36:27
 *
 * Model untuk supplier yang mewarisi fungsi-fungsi dari BaseModel
 * Table: supplier
 * Fields: 10 fields
 * Database: PostgreSQL
 */
class SupplierModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel supplier
    const validFields = [
      'supplier_id',
      'supplier_code',
      'supplier_name',
    ...
    const datatablesWhere = ["supplier_code","supplier_name","contact_person","phone","email","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('supplier', validFields, datatablesWhere);
    ...
    this.readSource = 'supplier'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'supplier'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan supplier
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk supplier
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'supplier-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'supplier_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT supplier_id, supplier_id FROM ' + this.getLookupSource() + ' WHERE upper(a.supplier_id) LIKE upper($1) ORDER BY a.supplier_id';
      ...
      const result = data.map(item => ({
        id: item.supplier_id,
        text: item.supplier_id
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (SupplierModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["supplier_id","supplier_code","supplier_name","contact_person","phone","email","address","city","country","is_active"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'supplier_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.supplier_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (SupplierModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'supplier_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT supplier_id, supplier_id FROM ' + this.getLookupSource() + ' ORDER BY a.supplier_id';
      ...
      return data.map(item => {
        ...
        if (item.supplier_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (SupplierModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['supplier_id', 'supplier_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["supplier_code","supplier_name"];
      let selectClause = 'supplier_id';
      let textField = 'supplier_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'supplier_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'supplier_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'supplier_id') return null;
      ...
      console.log('=== DEBUG SUPPLIER LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.supplier_id,
        text: item[textFieldName] || item.supplier_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (SupplierModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["supplier_code","supplier_name"],
      dateFields: [],
      requiredFields: ["supplier_code","supplier_name","email"],
      primaryTextField: 'supplier_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk supplier
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response supplier
   */
