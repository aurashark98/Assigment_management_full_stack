...
/**
 * Customer Model - Auto-generated on 2026-04-15 00:36:36
 *
 * Model untuk customer yang mewarisi fungsi-fungsi dari BaseModel
 * Table: customer
 * Fields: 10 fields
 * Database: PostgreSQL
 */
class CustomerModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel customer
    const validFields = [
      'customer_id',
      'customer_code',
      'customer_name',
    ...
    const datatablesWhere = ["customer_code","customer_name","contact_person","phone","email","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('customer', validFields, datatablesWhere);
    ...
    this.readSource = 'customer'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'customer'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan customer
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk customer
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'customer-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'customer_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT customer_id, customer_id FROM ' + this.getLookupSource() + ' WHERE upper(a.customer_id) LIKE upper($1) ORDER BY a.customer_id';
      ...
      console.error('Error in getLookupData (CustomerModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["customer_id","customer_code","customer_name","contact_person","phone","email","address","city","country","is_active"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'customer_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.customer_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (CustomerModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'customer_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT customer_id, customer_id FROM ' + this.getLookupSource() + ' ORDER BY a.customer_id';
      ...
      return data.map(item => {
        ...
        if (item.customer_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (CustomerModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['customer_id', 'customer_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["customer_code","customer_name"];
      let selectClause = 'customer_id';
      let textField = 'customer_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'customer_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'customer_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'customer_id') return null;
      ...
      console.log('=== DEBUG CUSTOMER LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.customer_id,
        text: item[textFieldName] || item.customer_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (CustomerModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["customer_code","customer_name"],
      dateFields: [],
      requiredFields: ["customer_code","customer_name","email"],
      primaryTextField: 'customer_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk customer
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response customer
   */
