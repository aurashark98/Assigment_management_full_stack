...
/**
 * Category Model - Auto-generated on 2026-04-15 00:36:06
 *
 * Model untuk category yang mewarisi fungsi-fungsi dari BaseModel
 * Table: category
 * Fields: 5 fields
 * Database: PostgreSQL
 */
class CategoryModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel category
    const validFields = [
      'category_id',
      'category_code',
      'category_name',
    ...
    super('category', validFields, datatablesWhere);
    ...
    this.readSource = 'category'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'category'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan category
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk category
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'category-datatables.sql');
  ...
  async getLookupData(search) {
    try {
      ...
      console.error('Error in getLookupData (CategoryModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["category_id","category_code","category_name","description","is_active"].includes(key)) {
      ...
      console.error('Error in getLookupDataDynamic (CategoryModel):', error);
      throw error;
    }
  }
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      console.error('Error in getStaticLookupData (CategoryModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      console.log('=== DEBUG CATEGORY LOOKUP WITH FILTER ===');
      ...
      console.error('Error in getLookupDataWithFilter (CategoryModel):', error);
      throw error;
    }
  }
  ...
  /**
   * Override formatResponseData untuk category
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response category
   */
