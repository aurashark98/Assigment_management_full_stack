...
/**
 * Warehouse Model - Auto-generated on 2026-04-15 00:36:18
 *
 * Model untuk warehouse yang mewarisi fungsi-fungsi dari BaseModel
 * Table: warehouse
 * Fields: 8 fields
 * Database: PostgreSQL
 */
class WarehouseModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel warehouse
    const validFields = [
      'warehouse_id',
      'warehouse_code',
      'warehouse_name',
    ...
    const datatablesWhere = ["warehouse_code","warehouse_name","warehouse_type","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('warehouse', validFields, datatablesWhere);

    // Setup primary key dari payload atau fallback ke fieldName pertama
    this.primaryKey = 'warehouse_id';

    // Setup viewName untuk operasi read jika berbeda dari tableName
    this.viewName = 'warehouse';
    this.readSource = 'warehouse'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'warehouse'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan warehouse
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk warehouse
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'warehouse-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'warehouse_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT warehouse_id, warehouse_id FROM ' + this.getLookupSource() + ' WHERE upper(a.warehouse_id) LIKE upper($1) ORDER BY a.warehouse_id';
      ...
      const result = data.map(item => ({
        ...
        text: item.warehouse_id
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (WarehouseModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["warehouse_id","warehouse_code","warehouse_name","warehouse_type","address","city","capacity","is_active"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'warehouse_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.warehouse_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (WarehouseModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'warehouse_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT warehouse_id, warehouse_id FROM ' + this.getLookupSource() + ' ORDER BY a.warehouse_id';
      ...
      return data.map(item => {
        ...
        if (item.warehouse_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (WarehouseModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['warehouse_id', 'warehouse_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["warehouse_code","warehouse_name"];
      let selectClause = 'warehouse_id';
      let textField = 'warehouse_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'warehouse_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'warehouse_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'warehouse_id') return null;
      ...
      console.log('=== DEBUG WAREHOUSE LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.warehouse_id,
        text: item[textFieldName] || item.warehouse_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (WarehouseModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["warehouse_code","warehouse_name"],
      dateFields: [],
      requiredFields: ["warehouse_code","warehouse_name"],
      primaryTextField: 'warehouse_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk warehouse
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response warehouse
   */
