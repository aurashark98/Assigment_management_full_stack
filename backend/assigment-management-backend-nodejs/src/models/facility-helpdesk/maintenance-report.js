...
/**
 * MaintenanceReport Model - Auto-generated on 2026-04-15 01:43:40
 *
 * Model untuk maintenance-report yang mewarisi fungsi-fungsi dari BaseModel
...
class MaintenanceReportModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel maintenance-report
    const validFields = [
      'report_id',
      'report_number',
      'reporter_id',
    ...
    const datatablesWhere = ["report_number","title","reporter_name","category_name","facility_name","status","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('maintenance_report', validFields, datatablesWhere);
    ...
    this.viewName = 'maintenance_report';
    this.readSource = 'maintenance_report'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'maintenance_report'; // Source untuk operasi write (add, update, delete)
    ...
    // Model metadata
    this.modelMetadata = {
      endpointName: 'maintenance-report',
      moduleName: 'facility-helpdesk',
      tableName: 'maintenance_report',
      viewName: 'maintenance_report',
      fieldCount: 25,
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan maintenance-report
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk maintenance-report
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'maintenance-report-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'report_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT report_id, report_id FROM ' + this.getLookupSource() + ' WHERE upper(a.report_id) LIKE upper($1) ORDER BY a.report_id';
      ...
      console.error('Error in getLookupData (MaintenanceReportModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'report_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.report_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (MaintenanceReportModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'report_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT report_id, report_id FROM ' + this.getLookupSource() + ' ORDER BY a.report_id';
      ...
      console.error('Error in getStaticLookupData (MaintenanceReportModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['report_id', 'report_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["title"];
      let selectClause = 'report_id';
      let textField = 'report_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'report_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'report_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'report_id') return null;
      ...
      console.log('=== DEBUG MAINTENANCE-REPORT LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.report_id,
        text: item[textFieldName] || item.report_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (MaintenanceReportModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["title"],
      dateFields: ["started_at","created_at","updated_at"],
      requiredFields: ["title"],
      primaryTextField: 'report_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk maintenance-report
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response maintenance-report
   */
...
module.exports = new MaintenanceReportModel();