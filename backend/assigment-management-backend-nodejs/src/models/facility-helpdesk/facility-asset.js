...
/**
 * FacilityAsset Model - Auto-generated on 2026-04-15 01:43:38
 *
 * Model untuk facility-asset yang mewarisi fungsi-fungsi dari BaseModel
 * Table: facility_asset
 * Fields: 15 fields
 * Database: PostgreSQL
 */
class FacilityAssetModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel facility-asset
    const validFields = [
      'facility_id',
      'facility_code',
      'facility_name',
    ...
    const datatablesWhere = ["facility_code","facility_name","facility_type","room_name","location_detail","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('facility_asset', validFields, datatablesWhere);

    // Setup primary key dari payload atau fallback ke fieldName pertama
    this.primaryKey = 'facility_id';

    // Setup viewName untuk operasi read jika berbeda dari tableName
    this.viewName = 'facility_asset';
    this.readSource = 'facility_asset'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'facility_asset'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan facility-asset
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk facility-asset
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'facility-asset-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'facility_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT facility_id, facility_id FROM ' + this.getLookupSource() + ' WHERE upper(a.facility_id) LIKE upper($1) ORDER BY a.facility_id';
      ...
      const result = data.map(item => ({
        id: item.facility_id,
        text: item.facility_id
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (FacilityAssetModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      const textFields = lookupConfig && lookupConfig.searchFields.length > 0
        ...
        : ["facility_code","facility_name","room_name"];
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["facility_id","facility_code","facility_name","facility_type","floor_level","room_name","location_detail","brand","model","serial_number","condition_status","is_active","notes","created_at","updated_at"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'facility_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.facility_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (FacilityAssetModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'facility_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT facility_id, facility_id FROM ' + this.getLookupSource() + ' ORDER BY a.facility_id';
      ...
      return data.map(item => {
        ...
        if (item.facility_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (FacilityAssetModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['facility_id', 'facility_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["facility_code","facility_name","room_name"];
      let selectClause = 'facility_id';
      let textField = 'facility_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'facility_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'facility_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'facility_id') return null;
      ...
      console.log('=== DEBUG FACILITY-ASSET LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.facility_id,
        text: item[textFieldName] || item.facility_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (FacilityAssetModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["facility_code","facility_name","room_name"],
      dateFields: ["created_at","updated_at"],
      requiredFields: ["facility_code","facility_name","room_name"],
      primaryTextField: 'facility_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk facility-asset
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response facility-asset
   */
...
module.exports = new FacilityAssetModel();