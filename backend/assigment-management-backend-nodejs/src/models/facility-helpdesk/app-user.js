...
/**
 * AppUser Model - Auto-generated on 2026-04-15 01:43:10
 *
...
class AppUserModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel app-user
    const validFields = [
      'user_id',
      'employee_code',
      'full_name',
    ...
    const datatablesWhere = ["employee_code","full_name","email","role","department","job_title","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('app_user', validFields, datatablesWhere);
    ...
    this.viewName = 'app_user';
    this.readSource = 'app_user'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'app_user'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan app-user
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk app-user
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'app-user-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'user_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT user_id, user_id FROM ' + this.getLookupSource() + ' WHERE upper(a.user_id) LIKE upper($1) ORDER BY a.user_id';
      ...
      console.error('Error in getLookupData (AppUserModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      const textFields = lookupConfig && lookupConfig.searchFields.length > 0
        ...
        : ["employee_code","full_name","job_title"];
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["user_id","employee_code","full_name","email","phone","password_hash","role","department","job_title","is_active","last_login_at","created_at","updated_at"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'user_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.user_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (AppUserModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'user_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT user_id, user_id FROM ' + this.getLookupSource() + ' ORDER BY a.user_id';
      ...
      console.error('Error in getStaticLookupData (AppUserModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['user_id', 'user_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["employee_code","full_name","job_title"];
      let selectClause = 'user_id';
      let textField = 'user_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'user_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'user_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'user_id') return null;
      ...
      console.log('=== DEBUG APP-USER LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.user_id,
        text: item[textFieldName] || item.user_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (AppUserModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["employee_code","full_name","job_title"],
      dateFields: ["created_at","updated_at"],
      requiredFields: ["employee_code","full_name","email","job_title"],
      primaryTextField: 'user_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk app-user
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response app-user
   */
...
module.exports = new AppUserModel();