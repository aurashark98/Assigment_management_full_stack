...
/**
 * ReportAttachment Model - Auto-generated on 2026-04-15 01:43:41
 *
 * Model untuk report-attachment yang mewarisi fungsi-fungsi dari BaseModel
 * Table: report_attachment
 * Fields: 9 fields
 * Database: PostgreSQL
 */
class ReportAttachmentModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel report-attachment
    const validFields = [
      'attachment_id',
      'report_id',
      'attachment_type',
    ...
    const datatablesWhere = ["file_name","attachment_type","report_number","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('report_attachment', validFields, datatablesWhere);
    ...
    this.viewName = 'report_attachment';
    this.readSource = 'report_attachment'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'report_attachment'; // Source untuk operasi write (add, update, delete)
    ...
    // Model metadata
    this.modelMetadata = {
      endpointName: 'report-attachment',
      moduleName: 'facility-helpdesk',
      tableName: 'report_attachment',
      viewName: 'report_attachment',
      fieldCount: 9,
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan report-attachment
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk report-attachment
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'report-attachment-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'attachment_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT attachment_id, attachment_id FROM ' + this.getLookupSource() + ' WHERE upper(a.attachment_id) LIKE upper($1) ORDER BY a.attachment_id';
      ...
      const result = data.map(item => ({
        ...
        text: item.attachment_id
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (ReportAttachmentModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["attachment_id","report_id","attachment_type","file_name","file_url","mime_type","file_size_bytes","uploaded_by","created_at"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'attachment_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.attachment_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (ReportAttachmentModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'attachment_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT attachment_id, attachment_id FROM ' + this.getLookupSource() + ' ORDER BY a.attachment_id';
      ...
      return data.map(item => {
        ...
        if (item.attachment_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (ReportAttachmentModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['attachment_id', 'attachment_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["file_name"];
      let selectClause = 'attachment_id';
      let textField = 'attachment_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'attachment_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'attachment_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'attachment_id') return null;
      ...
      console.log('=== DEBUG REPORT-ATTACHMENT LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.attachment_id,
        text: item[textFieldName] || item.attachment_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (ReportAttachmentModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["file_name"],
      dateFields: ["created_at"],
      requiredFields: ["file_name"],
      primaryTextField: 'attachment_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk report-attachment
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response report-attachment
   */
...
module.exports = new ReportAttachmentModel();