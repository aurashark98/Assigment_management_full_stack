...
/**
 * ReportActivity Model - Auto-generated on 2026-04-15 01:43:44
 *
 * Model untuk report-activity yang mewarisi fungsi-fungsi dari BaseModel
 * Table: report_activity
 * Fields: 8 fields
 * Database: PostgreSQL
 */
class ReportActivityModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel report-activity
    const validFields = [
      'activity_id',
      'report_id',
      'actor_id',
    ...
    const datatablesWhere = ["activity_type","report_number","actor_name","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('report_activity', validFields, datatablesWhere);
    ...
    this.viewName = 'report_activity';
    this.readSource = 'report_activity'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'report_activity'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan report-activity
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk report-activity
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'report-activity-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'activity_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT activity_id, activity_id FROM ' + this.getLookupSource() + ' WHERE upper(a.activity_id) LIKE upper($1) ORDER BY a.activity_id';
      ...
      console.error('Error in getLookupData (ReportActivityModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["activity_id","report_id","actor_id","activity_type","status_from","status_to","activity_note","activity_at"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'activity_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.activity_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (ReportActivityModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'activity_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT activity_id, activity_id FROM ' + this.getLookupSource() + ' ORDER BY a.activity_id';
      ...
      return data.map(item => {
        ...
        if (item.activity_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (ReportActivityModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['activity_id', 'activity_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = [];
      let selectClause = 'activity_id';
      let textField = 'activity_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'activity_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'activity_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'activity_id') return null;
      ...
      console.log('=== DEBUG REPORT-ACTIVITY LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.activity_id,
        text: item[textFieldName] || item.activity_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (ReportActivityModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      dateFields: [],
      requiredFields: [],
      primaryTextField: 'activity_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk report-activity
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response report-activity
   */
...
module.exports = new ReportActivityModel();