...
/**
 * ReportComment Model - Auto-generated on 2026-04-15 01:43:43
 *
 * Model untuk report-comment yang mewarisi fungsi-fungsi dari BaseModel
 * Table: report_comment
 * Fields: 6 fields
 * Database: PostgreSQL
 */
class ReportCommentModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel report-comment
    const validFields = [
      'comment_id',
      'report_id',
      'author_id',
    ...
    const datatablesWhere = ["comment_text","report_number","author_name","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('report_comment', validFields, datatablesWhere);
    ...
    this.viewName = 'report_comment';
    this.readSource = 'report_comment'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'report_comment'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan report-comment
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk report-comment
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'report-comment-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'comment_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT comment_id, comment_id FROM ' + this.getLookupSource() + ' WHERE upper(a.comment_id) LIKE upper($1) ORDER BY a.comment_id';
      ...
      const result = data.map(item => ({
        id: item.comment_id,
        text: item.comment_id
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (ReportCommentModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["comment_id","report_id","author_id","comment_text","is_internal","created_at"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'comment_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.comment_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (ReportCommentModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'comment_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT comment_id, comment_id FROM ' + this.getLookupSource() + ' ORDER BY a.comment_id';
      ...
      return data.map(item => {
        ...
        if (item.comment_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (ReportCommentModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['comment_id', 'comment_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["comment_text"];
      let selectClause = 'comment_id';
      let textField = 'comment_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'comment_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'comment_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'comment_id') return null;
      ...
      console.log('=== DEBUG REPORT-COMMENT LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.comment_id,
        text: item[textFieldName] || item.comment_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (ReportCommentModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      requiredFields: [],
      primaryTextField: 'comment_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk report-comment
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response report-comment
   */
...
module.exports = new ReportCommentModel();