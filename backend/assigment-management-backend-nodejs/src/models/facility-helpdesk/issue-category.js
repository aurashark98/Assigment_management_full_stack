...
/**
 * IssueCategory Model - Auto-generated on 2026-04-15 01:43:36
 *
 * Model untuk issue-category yang mewarisi fungsi-fungsi dari BaseModel
 * Table: issue_category
 * Fields: 8 fields
 * Database: PostgreSQL
 */
class IssueCategoryModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel issue-category
    const validFields = [
      'category_id',
      'category_code',
      'category_name',
    ...
    const datatablesWhere = ["category_code","category_name","description","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('issue_category', validFields, datatablesWhere);
    ...
    this.viewName = 'issue_category';
    this.readSource = 'issue_category'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'issue_category'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan issue-category
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk issue-category
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'issue-category-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'category_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT category_id, category_id FROM ' + this.getLookupSource() + ' WHERE upper(a.category_id) LIKE upper($1) ORDER BY a.category_id';
      ...
      console.error('Error in getLookupData (IssueCategoryModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["category_id","category_code","category_name","description","is_active","sort_order","created_at","updated_at"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'category_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.category_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (IssueCategoryModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'category_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT category_id, category_id FROM ' + this.getLookupSource() + ' ORDER BY a.category_id';
      ...
      return data.map(item => {
        ...
        if (item.category_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (IssueCategoryModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['category_id', 'category_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["category_code","category_name"];
      let selectClause = 'category_id';
      let textField = 'category_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'category_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'category_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'category_id') return null;
      ...
      console.log('=== DEBUG ISSUE-CATEGORY LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.category_id,
        text: item[textFieldName] || item.category_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (IssueCategoryModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["category_code","category_name"],
      dateFields: ["created_at","updated_at"],
      requiredFields: ["category_code","category_name"],
      primaryTextField: 'category_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk issue-category
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response issue-category
   */
...
module.exports = new IssueCategoryModel();